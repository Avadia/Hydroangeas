    ...
    public Requester request(Route.CompiledRoute compiledRoute) {
        ...
        switch (method) {
            case GET:
                ...
                break;
            case POST:
                builder.post(RequestBody.create(MediaType.parse("application/json"), compiledRoute.getJSONData() != null ? compiledRoute.getJSONData().toString().getBytes() : "".getBytes()));
                break;
            case PATCH:
                builder.patch(RequestBody.create(MediaType.parse("application/json"), compiledRoute.getJSONData() != null ? compiledRoute.getJSONData().toString().getBytes() : "".getBytes()));
                break;
            case DELETE:
                builder.delete(RequestBody.create(MediaType.parse("application/json"), compiledRoute.getJSONData() != null ? compiledRoute.getJSONData().toString().getBytes() : "".getBytes()));
                break;
        }
